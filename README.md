...
- It was mentioned in the FAQ that this exercise was designed with the intent that it should take a few hours, so I keep all receipts in local LRU cache in memory. Max capacity is 10000 Receipts.
- OpenAPI yaml definition was changed, since `type: string / format: time` is not a valid OpenAPI construct, and it causes undefined behavior in codegen. `format` was commented out in yaml. Judging by provided examples the field format is "HH:mm", input validated manually.
